...
<pre class="shell">
  ...
  Import/Export management:
    ...
    export     Export contents of a table/view/query to (csv, json, yaml, ruby, xml)
    import     Import contents of a table from (csv, json, yaml, ruby)
</pre>

h3. Getting started

'dba' works with its own ~/.dbagile and ~/.dbagile_history files for managing connection configurations and command history, respectively. Assuming an existing postgresql database named 'contacts' on your machine (adapt all parameters, of course) and an existing 'people' table inside the database, try the following commands in a shell:
...
<pre>
  DbAgile::Command::add %w{contacts postgres://user:password@localhost/contacts}
  DbAgile::Command::list
  DbAgile::Command::ping
  DbAgile::Command::show %w{--pretty people}
  DbAgile::Command::export %w{--csv people}
</pre>
