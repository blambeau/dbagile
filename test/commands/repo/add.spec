shared_examples_for("The repo:add command") do
  ...
  it "should return the new database" do
    dba.repo_add(%w{test sqlite://test.db}).should be_kind_of(DbAgile::Core::Database)
  end
  
  it "should accept a sqlite relative database" do
    dba.repo_add(%w{test test.db}).should be_kind_of(DbAgile::Core::Database)
  end
  ...
  it "should raise an error when the name is already in use" do
    ...
    lambda{ dba.repo_add(%w{test sqlite://test.db}) }.should raise_error(DbAgile::DatabaseNameConflictError)
  end
